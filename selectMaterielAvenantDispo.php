        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                if ($disponibilte == 'disponibile') {
                    ...
                    echo '<option disabled style="background-color:red; color:white;" value="' . $row['id_materiels_agence'] . '">' . $row['code_materiel'] . '-' . $row['designation'] . '-' . $row['num_serie_materiels'] . ' Non Disponible</option>';
                }
            }
        }
        ?>
