...
if (isset($_GET['id'])){
  ...
  $id_contrat = $_GET['id'];
  $query = "SELECT CA.id_contrat_avenant,CA.debut_contrat_avenant,CA.fin_contrat_avenant,CA.id_voiture_avenant,CA.id_contrat_client,
  C.id_contrat,C.moyen_caution,C.caution,C.cautioncheque,C.num_cheque_caution,C.num_cb_caution,C.duree,C.id_client,C.type_location,C.num_contrat,
  ...
  LEFT JOIN voiture AS V on CA.id_voiture_avenant = V.id_voiture
  ...
  $result = mysqli_query($conn, $query);
          if ($result->num_rows > 0) {
              while ($row = $result->fetch_assoc()) {
                  ...
                  $Contrat_date_debut = $row['debut_contrat_avenant'];
                  $Contrat_date_debut = date("d-m-Y", strtotime($Contrat_date_debut));
                  $Contrat_date_fin = $row['fin_contrat_avenant'];
...
$pdf->AddPage();
define('EURO',chr(128));
$pdf->SetTitle(utf8_decode("Avenant au Contrat Véhicule_N°':".$Contrat_number."_".$Client_name));
$pdf->Image('logok2.jpg',10,15,20,15);
$pdf->SetFont('Arial','B',8);
$pdf->Cell(50,4,utf8_decode('Avenant au Contrat de location N°'). $Contrat_number,0,2,'',false);
...
// $pdf->Write(7,"Cet avenant prend effet du ");
// $pdf->SetTextColor(255,0,0);
// $pdf->Write(7,$Contrat_date_debut);
// $pdf->SetTextColor(0);
// $pdf->Write(7," au ");
// $pdf->SetTextColor(255,0,0);
// $pdf->Write(7,$Contrat_date_fin);
// $pdf->SetTextColor(0);
// $pdf->Write(7,utf8_decode(", suite à la mise à disposition du véhicule immatriculé "));
// $pdf->SetTextColor(255,0,0);
// $pdf->Write(7,$Vehicule_imm);
// $pdf->SetTextColor(0);
// $pdf->Write(7,utf8_decode(" ,le temps de l'intervention sur le véhicule prévu initialement sur votre contrat de location. "));

$texte22 = "Cet avenant prend effet du ".$Contrat_date_debut." au ".$Contrat_date_fin.", suite à la mise à disposition du véhicule immatriculé ".$Vehicule_imm." ,le temps de l'intervention sur le véhicule prévu initialement sur votre contrat de location. ";
...
$pdf->Output('I',utf8_decode("Avenant au Contrat Véhicule_N°:".$Contrat_number."_".$Client_name.".pdf"));
}
else {
  echo "erreur";
}
