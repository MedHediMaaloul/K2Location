...
CL.nom_entreprise,CL.nom, CL.email, CL.tel, CL.adresse, 
      C.id_contrat, C.prix,
    ...
    LEFT JOIN materiel_contrat_client AS CM ON CM.id_contrat_avenant = CA.id_contrat_avenant
    LEFT JOIN client AS CL ON CL.id_client = C.id_client 
    LEFT JOIN composant_materiels_contrat AS CC ON CC.id_contrat_avenant =CM.id_contrat_avenant
...
$pdf->AddPage();
define('EURO',chr(128));
$pdf->SetTitle(utf8_decode("Avenant au Contrat Matériel_N°:".$Contrat_number."_".$Client_name));
...
$pdf->SetFont('Arial','',8);
$pdf->SetTextColor(0);
$texte21 = "Cet avenant prend effet du ".$Contrat_date_debut." au ".$Contrat_date_fin.", suite à la mise à disposition du matériel ".$Materiel_Designation." dont le N°de série ".$Materiel_num_serie.", le temps de l'intervention, sur le matériel prévu initialement sur votre contrat de location";
...
$texte7 = "Pour les contrats avec engagement (minimum 6 mois), toute rupture de contrat, engendrera des frais de résiliation à hauteur de 30% de la totalité des factures restantes. ";
$pdf->MultiCell(0,5,utf8_decode("Le locataire verse à K2, une somme de ").$Contrat_caution ." ".chr(128).utf8_decode($texte6)."\n".$texte61."\n".utf8_decode($texte7));
$pdf->Ln(5);
$pdf->VerifPage();
$pdf->Cell(80, 0,utf8_decode("La sous-location du matériel par le locataire à un tiers est exclue ."));
$pdf->Ln(10);
$pdf->VerifPage();
$pdf->SetFont('Arial','B',7);
$pdf->SetTextColor(0);
...
$pdf->Output('I',utf8_decode("Avenant au Contrat Matériel_N°:".$Contrat_number."_".$Client_name.".pdf"));
  }
else {
  echo "erreur";
} 
